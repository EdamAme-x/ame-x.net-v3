...
export function ProfileCard() {
	...
	const fetchFollowing = async () => {
		...
		const nitterDOM = new DOMParser().parseFromString(
			await nitterRes.text(),
			"text/html",
		)

		const nitterFollowing = parseInt((nitterDOM.querySelector("body > div > div > div.profile-tab.sticky > div > div.profile-card-extra > div.profile-card-extra-links > ul > a:nth-child(3) > li > span.profile-stat-num")?.innerHTML ?? "2189").replace(/\,/g, ""))
		
		const zennRes = await fetch("/api/proxy?url=" + zenn);
		const zennDOM = new DOMParser().parseFromString(
			await zennRes.text(),
			"text/html",
		)

		const zennFollowing = parseInt((zennDOM.querySelector("#__next > header:nth-child(2) > div > div > div > div > div > button:nth-child(2) > span")?.innerHTML ?? "2189").replace(/\,/g, "")) + parseInt((document.querySelector("#__next > header:nth-child(2) > div > div > div > div > div > button:nth-child(1) > span")?.innerHTML ?? "2189").replace(/\,/g, ""))

		const githubRes = await fetch(github);
		const githubFollowing = parseInt((await githubRes.json()).followers);

		localStorage.setItem("_cache_following", JSON.stringify(
			{
